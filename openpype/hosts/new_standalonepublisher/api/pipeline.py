...
class HostContext:
    ...
    def get_context_json_path(cls):
        if cls._context_json_path is None:
            output_file = tempfile.NamedTemporaryFile(
                mode="w", prefix="standalonep_", suffix=".json"
            )
            output_file.close()
    ...
    @classmethod
    def save_project_name(cls, project_name):
...
def install():
    os.environ["AVALON_APP"] = "new_standalonepublisher"

    pyblish.api.register_host("new_standalonepublisher")
...
def save_project_name(project_name):
    ...
    HostContext.save_project_name(project_name)
